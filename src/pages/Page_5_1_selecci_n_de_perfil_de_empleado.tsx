...
const Page_5_1_selecci_n_de_perfil_de_empleado: React.FC = () => {
    ...
    const handleQuickDeploy = async (roleName: string) => {
        const { data: { user } } = await supabase.auth.getUser();
        if (!user) { alert('Debes iniciar sesión real primero.'); return; }

        setDeploying(roleName);
        const newAgent = {
            name: `Agente de ${roleName}`,
            role: roleName,
            status: 'Activo',
            avatar_icon: roleName.includes('Ventas') ? 'rocket_launch' : (roleName.includes('Soporte') ? 'support_agent' : 'account_tree'),
            color_class: roleName.includes('Ventas') ? 'border-primary' : (roleName.includes('Soporte') ? 'border-secondary' : 'border-tertiary'),
            user_id: user.id
        };

        await supabase.from('ai_agents').insert([newAgent]);
        setDeploying(null);
        alert(`¡${roleName} contratado exitosamente!`);
        navigate('/3_vista_kanban_multi_estado');
    };
    return (
            ...
            <section className="max-w-6xl mx-auto w-full px-8 pt-28 pb-12">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                    ...
                    <div className="group relative flex flex-col bg-surface-container-low rounded-[2rem] p-8 transition-all hover:bg-surface-container-highest shadow-sm hover:shadow-xl hover:-translate-y-1">
                        ...
                        <button disabled={deploying === 'Ventas'} onClick={() => handleQuickDeploy('Ventas')} className="w-full disabled:opacity-50 bg-primary text-on-primary py-4 rounded-2xl font-bold text-sm tracking-wide shadow-lg shadow-primary/20 hover:brightness-110 transition-all">
                            {deploying === 'Ventas' ? 'Desplegando...' : 'Seleccionar Perfil'}
                        </button>
                    </div>
                    {/*  Agente de Soporte  */}
                    <div className="group relative flex flex-col bg-surface-container-low rounded-[2rem] p-8 transition-all hover:bg-surface-container-highest shadow-sm hover:shadow-xl hover:-translate-y-1">
                        ...
                        <button disabled={deploying === 'Soporte'} onClick={() => handleQuickDeploy('Soporte')} className="w-full disabled:opacity-50 bg-on-surface text-surface py-4 rounded-2xl font-bold text-sm tracking-wide shadow-lg shadow-black/5 hover:brightness-125 transition-all">
                            {deploying === 'Soporte' ? 'Desplegando...' : 'Seleccionar Perfil'}
                        </button>
                    </div>
                    ...
                    <div className="group relative flex flex-col bg-surface-container-low rounded-[2rem] p-8 transition-all hover:bg-surface-container-highest shadow-sm hover:shadow-xl hover:-translate-y-1">
                        ...
                        <button disabled={deploying === 'Administrativo'} onClick={() => handleQuickDeploy('Administrativo')} className="w-full disabled:opacity-50 bg-surface-container-highest text-on-surface-variant py-4 rounded-2xl font-bold text-sm tracking-wide hover:bg-outline-variant/40 transition-all border border-outline-variant/30">
                            {deploying === 'Administrativo' ? 'Desplegando...' : 'Seleccionar Perfil'}
                        </button>
                    </div>
                </div>
